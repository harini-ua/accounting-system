...
class BonusesSalesManagersDataTable extends DataTable
{
    const COLUMNS = [
        'person',
        'bonus',
    ...
    public function dataTable($query)
    {
        ...
        $dataTable->addColumn('total', function(Person $model) {
            ...
            return view('pages.bonuses.table.sales-manager._total', compact('model', 'data', 'currency'));
        });
    ...
    private function monthColumns(): array
    {
        ...
        foreach($this->period() as $month) {
            $columns[] = Column::make(strtolower($month->monthName))
                ->title(view('pages.bonuses.table.sales-manager._head', compact('month', 'year'))->render())
                ->orderable(false)
                ->searchable(false);
        }
    ...
    private function addMonthColumnsToDatatable($dataTable): array
    {
        ...
        foreach($this->period() as $month) {
            ...
            $dataTable->addColumn($monthName, static function(Person $model) use ($month, $monthName, $currency) {
                ...
                return view('pages.bonuses.table.sales-manager._month', compact('data', 'currency', 'month'));
            });
        }
